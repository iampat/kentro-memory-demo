...
- **Conflicts are stored, not resolved at write time. Resolution is a read-time concern via the `resolver=` parameter.** Default `resolver=AutoResolver()` uses the rule from the schema. Built-in resolvers: `RawResolver()`, `LatestWriteResolver()`, `PreferAgent("...")`, **`SkillResolver(prompt=...)`** for domain-aware policies (e.g., "written sources outweigh verbal"). SkillResolver wraps an LLM call with structured output; if the skill cannot decide, it returns `UNRESOLVED` with a reason — the SDK never asks back. The agent layer handles the unresolved case.
