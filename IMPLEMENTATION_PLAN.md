...
| — | Resolver editor moved out of lineage drawer into its own stacked overlay (PR 35's inline form was overlapping candidates and clipping past the rail) | `done` | 36 |
...
#### PR 36 — Resolver editor in its own stacked drawer (deep slot, right: 960px) — **DONE**

**Built (2026-05-04):** PR 35 shipped an inline resolver editor that lived under the RESOLVER chip inside `LineageDrawer`, but the chip's column has no fixed width, so the editor (`width: 100%` of that column) escaped past the drawer to the right (clipping the apply button), painted over the candidate value chips and SVG connectors, and broke the geometry of the flow it was meant to help reason about. Replaced it with a third stacked drawer pattern, same shape as `PolicyOverlay`'s left-stacked slot. New `K.ResolverDrawer` (`panels.jsx`) is 380px wide, pinned at `right: 960px` so its right edge meets the lineage drawer's left edge; reuses `.drawer` chrome with a new `.drawer-deep` modifier (`styles.css`) and a capture-phase ESC handler stacked above the lineage drawer's listener. The form body is the same 5-option select + conditional `agent_id` / `prompt` inputs as PR 35, hosted as drawer-body content (drawer-head supplies the title + close `esc` button). `LineageFlowLayout` lost its `editorOpen` state and inline render — the RESOLVER chip's `onClick` calls `onEditResolver({entity_type, field_name})` upward; `LineageFlow`/`LineageDrawer` pass it through to `app.jsx`. `app.jsx` adds `resolverTarget` state, an `openResolver` callback wired to `LineageDrawer.onEditResolver`, and a `lineageRefreshKey` counter bumped on `ResolverDrawer.onApplied` so the lineage drawer re-fetches (its read effect now includes `refreshKey` in deps) and the flow re-renders the new winner. Old `.resolver-editor` outer-card CSS deleted; row/actions/error rules retained for the form fields inside the new drawer. **End-to-end verified via chrome-devtools MCP:** open Customer · Acme Corp → click `deal_size` → click RESOLVER chip → resolver drawer slides in left of lineage drawer; switching type to `prefer_agent` reveals the agent_id input; setting `sales` + apply bumps rule version, closes the drawer, re-fetches lineage; re-opening pre-populates with the saved policy; close button dismisses only the resolver drawer. **194/194 unit tests pass; ruff + format clean.**
