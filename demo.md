...
4. **Conflict resolution:** A `SkillResolver` is installed for `deal_size` conflicts — domain policy: written outweighs verbal, latest among written wins.

The previously-recorded conflict from the steady state resolves itself live under the new domain policy. Both values stay in lineage; the canonical becomes the Wednesday email's $300K *because written outweighs verbal*, not just because it was newer.
...
- **Resolution:** `SkillResolver` — "written outweighs verbal" → email wins
- **Corroboration:** 2 sources

Voice-over: "Click any field — full lineage. Both source documents, the agent that wrote them, the rules in effect at the time, and the policy that resolved the conflict. Audit isn't a bolt-on — it's how memory works."
...
6. The wedge is a system, not a feature.
...
- **Three entity types** (Customer, Deal, AuditLog). Hand-coded YAML/JSON schemas.
...
- **Two conflict resolvers** — `LatestWriteResolver` (built-in, mechanical) and `SkillResolver` (LLM call with structured Pydantic output, takes a domain-policy prompt). The demo uses the latter as the centerpiece of Scene 4. SkillResolver also returns `UNRESOLVED` when it can't decide — that path needs to work in the Colab.
...
- **The UI:**
  ...
  - Bottom strip: lineage drawer (collapsed; opens on click). Conflict-record drawer same surface.
...
- Custom user-defined resolvers beyond `SkillResolver` (post-YC)
- Cost counter on screen (mention in voice-over only)
- Sign-up, multi-tenancy, anything operational
...
14. **Re-run the read.** Same conflict; new resolution. Now the email wins because of the policy, not the timestamp. `viz.lineage(...)` shows the resolution annotation.
...
The Colab does the work the recorded video can't fit. The `UNRESOLVED` cell is the cleanest demonstration of "the SDK never asks back" — the SkillResolver has a real reason it couldn't decide, returns it as a typed status, and the caller (the agent) decides what to do. The source-churn cell is the cleanest demonstration of why conflict-as-memory is architectural, not cosmetic.
