...
def _initial_ruleset() -> RuleSet:
    rules = [
        ...
        FieldReadRule(agent_id=SALES, entity_type="Customer", field_name="deal_size", allowed=True),
        FieldReadRule(agent_id=SALES, entity_type="Customer", field_name="sales_notes", allowed=True),
        FieldReadRule(agent_id=SALES, entity_type="Customer", field_name="support_tickets", allowed=True),
        # Sales — writes
        ...
        FieldReadRule(agent_id=CS, entity_type="Customer", field_name="support_tickets", allowed=True),
        # CS — writes
...
def _post_scene4_ruleset() -> RuleSet:
    ...
    base.append(FieldReadRule(
        agent_id=CS, entity_type="Customer", field_name="deal_size", allowed=False,
    ))
    # 2. Sales gains read access to AuditLog for Acme.
    base.append(EntityVisibilityRule(
        agent_id=SALES, entity_type="AuditLog", entity_key="Acme", allowed=True,
    ))
    # 3. CS write to support_tickets now requires manager approval.
    base.append(WriteRule(
        agent_id=CS,
...
def test_sales_can_read_customer_deal_size() -> None:
    d = evaluate_field_read(
        ...
        agent_id=SALES, ruleset=_initial_ruleset(),
    )
    if not d.allowed:
...
def test_cs_cannot_read_customer_deal_size() -> None:
    d = evaluate_field_read(
        entity_type="Customer", field_name="deal_size",
        agent_id=CS, ruleset=_initial_ruleset(),
    )
    if d.allowed:
...
def test_cs_can_read_support_tickets() -> None:
    d = evaluate_field_read(
        entity_type="Customer", field_name="support_tickets",
        agent_id=CS, ruleset=_initial_ruleset(),
    )
    if not d.allowed:
        raise AssertionError(f"expected allowed, got: {d}")
...
def test_auditlog_invisible_to_sales_initially() -> None:
    d = evaluate_entity_visibility(
        entity_type="AuditLog", entity_key="Acme",
        agent_id=SALES, ruleset=_initial_ruleset(),
    )
    if d.allowed:
...
def test_sales_can_write_any_deal_field_via_wildcard() -> None:
    d = evaluate_write(
        entity_type="Deal", field_name="value",
        agent_id=SALES, ruleset=_initial_ruleset(),
    )
    if not d.allowed:
...
def test_unknown_agent_denied_by_default() -> None:
    d = evaluate_field_read(
        entity_type="Customer", field_name="name",
        agent_id="random_other", ruleset=_initial_ruleset(),
    )
    if d.allowed:
...
def test_after_scene4_cs_cannot_read_deal_size_explicit_deny() -> None:
    d = evaluate_field_read(
        entity_type="Customer", field_name="deal_size",
        agent_id=CS, ruleset=_post_scene4_ruleset(),
    )
    if d.allowed:
...
def test_after_scene4_sales_can_read_acme_auditlog() -> None:
    d = evaluate_entity_visibility(
        entity_type="AuditLog", entity_key="Acme",
        agent_id=SALES, ruleset=_post_scene4_ruleset(),
    )
    if not d.allowed:
...
def test_after_scene4_sales_still_blocked_from_other_audit_keys() -> None:
    d = evaluate_entity_visibility(
        entity_type="AuditLog", entity_key="OtherCorp",
        agent_id=SALES, ruleset=_post_scene4_ruleset(),
    )
    if d.allowed:
